...
temperature, and per-disk filesystem free space + used % into Home Assistant as MQTT
...
2. HA UI → **Settings → Devices & Services → MQTT → Devices**. Should
   ...
   temperature, plus per-disk SMART temperature, filesystem free
   space (auto-rendered in GB/TB), and filesystem used %.
